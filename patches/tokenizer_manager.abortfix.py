...
class TokenizerManager(TokenizerControlMixin, TokenizerManagerScoreMixin):
    ...
    def abort_request(self, rid: str = "", abort_all: bool = False):
        ...
        if not abort_all and not rid:
            ...
            return
        # Always dispatch the abort to the scheduler, even when the local state
        # is already gone: on client disconnect the stream cleanup can pop
        # rid_to_state before the delayed abort task fires, and skipping the
        # dispatch here leaves the scheduler generating (and discarding) tokens
        # until the context limit. Scheduler-side abort is idempotent.
